plot_LG_hydro_plot_customQ <- function(Q, linekey, dat){
  ...
  p<- ggplot(Q)+
    ...
    geom_ribbon(
      ...
      aes(fake_date_customQ,ymin=min_acceptable,ymax=min_optimal,fill=fill_name), alpha=.2
    ) +
    geom_ribbon(
      data=dat %>%
        mutate(fill_name = 'optimal') %>%
        filter(month %in% 1:12),
      aes(fake_date_customQ,ymin=min_optimal,ymax=max_optimal,fill=fill_name), alpha=.2
    ) +
    geom_ribbon(
      data=dat %>%
        mutate(fill_name = 'high') %>%
        filter(month %in% 1:12),
      aes(fake_date_customQ,ymin=max_optimal,ymax=max_acceptable,fill=fill_name), alpha=.2
    ) +
