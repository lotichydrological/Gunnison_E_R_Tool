...
evaluate_duration_in_target_range_bc_baseflow <-  function(Q_df, target_meta_df,
  ...
  out<- target_meta_df %>% 
    ...
    group_by(rowid) %>% 
    group_split() %>% 
    map_df(calculate_duration_in_target_range, Q=annual_min_flow_Q,flow_tol =flow_tolerance)
